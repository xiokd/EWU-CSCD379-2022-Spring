...
export class WordleGame {
  ...
  get correctChars(){
    ...
    const temp = allLetters.filter(x => x.status === LetterStatus.Correct).map(x => x.char);
    return temp;
  }

  get wrongPlaceLetters(){
    const allLetters = this.words.map(x => x.letters).flat();
    const wrongPlaceLetters = allLetters.filter(x => x.status === LetterStatus.WrongPlace).map(x => x.char);
